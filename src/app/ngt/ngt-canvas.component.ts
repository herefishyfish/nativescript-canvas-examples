...
import { NgtDomEvent } from "angular-three/lib/events";
...
@Component({
  ...
  standalone: true,
  template: `
    <Canvas #glCanvas backgroundColor="blue" (ready)="initialize()"></Canvas>
  `,
  // imports: [NgxResize],
  providers: [
    // NgtStore,
...
export class NSNgtCanvas
  extends NgtSignalStore<NgtCanvasInputs>
  implements OnInit, OnChanges
{
  ...
  @ViewChild("glCanvas", { static: true, read: ViewContainerRef })
  ...
  get hbPointerEvents() {
    return this.select("eventSource")() !== this.#host.nativeElement
      ? "none"
      : "auto";
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      changes["sceneGraphInputs"] &&
      !changes["sceneGraphInputs"].firstChange &&
      this.#glRef
    ) {
