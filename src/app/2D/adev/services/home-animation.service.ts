...
import { Canvas as NSCanvas } from '@nativescript/canvas';
import {
  BREAKPOINT,
  BUILD_TEXT,
  CANVAS,
...
export class HomeAnimation {
   ...
   */
  async init(canvas: NSCanvas): Promise<void> {
   ...
   */
  private async initCanvas(canvas: NSCanvas): Promise<void> {
    this.canvas = new Canvas(canvas, this.document, window);
  }
